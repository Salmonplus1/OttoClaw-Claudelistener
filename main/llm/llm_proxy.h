...
/**
 * Reload LLM configuration from NVS.
 */
...
/**
 * Get the HTTP status code from the most recent LLM API call.
 */
int llm_get_last_http_status(void);

/**
 * Validate the LLM API key by sending a minimal request.
 * Returns ESP_OK if key is valid, ESP_ERR_INVALID_RESPONSE for 401/403/400,
 * ESP_FAIL for network errors, ESP_ERR_INVALID_STATE if no key configured.
 */
...
typedef struct {
    ...
    size_t text_len;
    llm_tool_call_t calls[OTTOCLAW_MAX_TOOL_CALLS];
    int call_count;
